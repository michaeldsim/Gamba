<script>
  ...
  function handleWagerInput(event) {
    ...
    wager =
      event.target.value === ''
        ? ''
        : validateInput(Number(event.target.value), 0, Number.MAX_SAFE_INTEGER)
    updatePotentialEarnings()
  }
  ...
  function updatePotentialEarnings() {
    ...
    potentialEarningsHigher =
      wager * calculateProfitMultiplier(winChanceHigher) - wager
    potentialEarningsLower =
      wager * calculateProfitMultiplier(winChanceLower) - wager
  }
...
<div class="container mt-5">
  <h1>High Low</h1>
  <p>
    Choose a number and guess whether the next number generated will be higher
    or lower (Range of 0-100). The lower the odds the higher the payout!
  </p>
  <div class="mb-3">
    <label for="chosenNumber" class="form-label"
      >Choose your number (1-99):</label
    >
    <input
      type="number"
      class="form-control"
      bind:value={chosenNumber}
      on:input={handleChosenNumberInput}
      min="1"
      max="99"
    />
  </div>
  <div class="mb-3">
    ...
    <input
      ...
      class="form-control"
      bind:value={wager}
      on:input={handleWagerInput}
      min="0"
    />
  </div>
  <button class="btn btn-primary me-2" on:click={() => makeGuess(true)}
    >Guess Higher</button
  >
  <button class="btn btn-secondary" on:click={() => makeGuess(false)}
    >Guess Lower</button
  >
